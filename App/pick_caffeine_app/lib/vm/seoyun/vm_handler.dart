...
class Order extends GetxController {
  ...
  Future<void> fetchStore(String id) async {
    try {
      final res = await http.get(
        Uri.parse("$baseUrl/select/purchase_list/storeinfo/$id"),
      );
      final data = json.decode(utf8.decode(res.bodyBytes));
      final List results = data['results'];
  ...
  Future<void> fetchDetailMenu(String id, String num) async {
    try {
      detailMenu.clear();
      final res = await http.get(
        Uri.parse("$baseUrl/select/detail_menu/$id/$num"),
      );
      final data = json.decode(utf8.decode(res.bodyBytes));
      final List results = data['results'];
  ...
  Future<void> fetchDetailMenuStore(String id, String num) async {
    try {
      ...
      final res = await http.get(
        Uri.parse("$baseUrl/select/detail_menu/store/$id/$num"),
      );
      final data = json.decode(utf8.decode(res.bodyBytes));
      final List results = data['results'];
    ...
    try {
      ...
      for (var store in results) {
        myStore.add({
          ...
          'image_1': store['image_1'],
          'store_like_count': store['store_like_count'],
